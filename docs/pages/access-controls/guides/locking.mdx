...
environment on an *ad hoc* basis, e.g. to disable a compromised user or
prevent access during cluster maintenance.

When a lock is in force, all interactions (like SSH/DB/k8s connections or
...
Locks are modeled as [resources](../../admin-guide.mdx#resources) with `kind:
