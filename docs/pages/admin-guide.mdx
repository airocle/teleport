...
*external* users via [Github](#github-oauth-20) or [Okta](enterprise/sso/okta.mdx) or any other
...
Each user identity is also assigned one or more administrator-defined *roles*, which allows for customized granular access control. For this guide, we'll assign new users to the default `admin`, `access`, and `editor` roles.
...
A cluster membership means that a node receives its own host certificate signed by the cluster's auth server. To receive a host certificate upon joining a cluster, a new Teleport host must present an *invite token*. An invite token also defines which role a new host can assume within a cluster: `auth`, `proxy`, `node`, `app`, `kube`, or `db`.
...
3. **Optional: [Enhanced Session Recording](./server-access/guides/bpf-session-recording.mdx)**
...
| lock | A [lock](./access-controls/guides/locking.mdx) for locking sessions and identities. |
...
- enable scp mode in the Ansible config file (ansible searches in `$ANSIBLE_CONFIG`, `./ansible.cfg`, `~/.ansible.cfg`, and finally `/etc/ansible/ansible.cfg`  in that order):
