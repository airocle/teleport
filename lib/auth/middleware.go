...
func (t *TLSServer) GetConfigForClient(info *tls.ClientHelloInfo) (*tls.Config, error) {
	...
	default:
		...
		// If Auth Service was dialed by SNI ALPN proxy (ProtocolAuth value present in SupportedProtos filed)
		// reset cluster name in order to  validate against all known CAs.
		// This is needed because in trusted cluster scenario the client uses root cluster cert with remote cluster name
		// encoded into SNI value.
